...
class SolverAgent(BaseAgent):
    ...
    _SYSTEM_PROMPT = """\
        ...
        Strategy: {strategy}
        Attempt {attempt} of {max_attempts}.{feedback_block}"""
    ...
    def _bind_tools(self, rag_available: bool):
        tools = _TOOLS_WITH_RAG if rag_available else _TOOLS
    ...
    def solver_agent(self, state: AgentState) -> dict:
        ...
        try:
            ...
            response = self._bind_tools(rag_available).invoke(messages)
            ...
            logger.info(f"[Solver] Working done | iter={iteration + 1} | calc={calc_used} | rag={rag_used}")
            return {
                "messages": [response],
